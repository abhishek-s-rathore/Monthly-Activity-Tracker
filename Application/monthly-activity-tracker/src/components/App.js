...
class App extends React.Component{
  ...
  window.addEventListener('beforeunload', this.handleUpdateLocalStorage)
    ...
    render(){
        ...
        return(
            <>
            <Header  key='header' enteredActivity={this.state.enteredActivity} handleChange={(response)=>this.handleChange(response)} handleSubmit={(response)=>this.handleSubmit(response)}/>
            <Activities  key='activities' activities={this.state.activities} handleDelete={(response)=>this.handleDelete(response)} handleSelectedDate={(activity, day)=>this.handleSelectedDate(activity, day)}/>
            </>
        );
    }
}
